...
const _WeatherForecast = (props)=>{
  ...
  const tableRow =(data)=>{return(
  <tr key={data}>{data.map((item)=>{return(<td key={item}>{String(item)}</td>)})}</tr>)}

  return(
    forecast.length?
    <React.Fragment>
      <h1>Prognoza 3-dniowa</h1>
      <table className ='forecast__table'><thead><tr>{forecastHeaders.map(item => <th key={item}>{item}</th>)}</tr></thead>
