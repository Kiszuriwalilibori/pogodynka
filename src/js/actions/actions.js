...
export function hideErrorMessage(type) {
  return {
...
export function getWeather(source, city, redirectSuccess) {
  return dispatch => {
    const NoDataHandle = (code, city) => {
      //code indicates where the failure happens(ws it when fetching target city weather, forecast, or group)
        ...
        fetch(createURLofWeatherAPI(code, city, source))
          ...
          .then(data => {
            if (validate[code](data)) {
              // if conection is OK, validates resulting data
