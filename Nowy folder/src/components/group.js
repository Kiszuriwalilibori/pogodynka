...
const _group = (props)=>{
    ...
    var createRow =(data, index)=>{return(<tr key={index}>{data.map((item)=>{return(createCell(item))})}</tr>)}
    ...
    return(
        ...
        <tbody>{ComparativeTable.map((item, index) => createRow(item, index))}</tbody></table>
        </React.Fragment>:null
